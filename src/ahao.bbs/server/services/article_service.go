...
func (s *articleService) Updates(id int64, colums map[string]interface{}) error {
	err := repositories.ArticleRepository.Updates(simple.DB(), id, colums)
	return err
}
...
func (s *articleService) GetArticleInIds(articleIds []int64) []model.Article {
	...
	simple.DB().Where("id in (?)", articleIds).Order("id desc").Find(&articleIds)
	return articles
}
